...
export async function GET() {
  ...
  const updatedAlerts = forgetAlerts.map(alert => {
    const studiedAt = alert.studiedAt ? new Date(alert.studiedAt).getTime() : Date.now()
    const h = (Date.now() - studiedAt) / (1000 * 60 * 60)
    return { ...alert, retentionPercentage: Math.max(0, Math.round(100 * Math.exp(-0.05 * h))) }
  }).sort((a, b) => a.retentionPercentage - b.retentionPercentage)

  const uniqueDays = new Set(
    recentLogs.map(l => l.studiedAt ? new Date(l.studiedAt).toDateString() : '')
  ).size

  const today = new Date().toDateString()
  const todayLogged = recentLogs.some(l => l.studiedAt && new Date(l.studiedAt).toDateString() === today)
  ...
  return NextResponse.json({
    ...
    profile: {
      ...
      focusCliff: profile?.focusCliff,
      completionPercent: Math.min(100, Math.round((uniqueDays / 7) * 100))
    },
    streak: { current: streak?.currentStreak || 0, longest: streak?.longestStreak || 0 },
    todayLogged,
    forgetAlerts: updatedAlerts,
    recentLogs,
    insightsUnlocked: uniqueDays >= 7 || insights.length > 0,
    daysUntilWrapped: dayOfWeek === 0 ? 0 : 7 - dayOfWeek,
  })
}
