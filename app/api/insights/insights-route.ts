...
export async function GET() {
  ...
  const uniqueDays = new Set(
    logs.map(l => l.studiedAt ? new Date(l.studiedAt).toDateString() : '')
  ).size
  ...
  if (unlocked && existing.length === 0 && profile) {
    try {
      const generated = await generateBrainInsights({
        quizAnswers: profile.quizAnswers as Record<string, string>,
        sessionLogs: logs.map(l => ({
          topic: l.topic,
          subject: l.subject,
          technique: l.technique,
          durationMinutes: l.durationMinutes,
          confidenceScore: l.confidenceScore,
          energyBefore: l.energyBefore,
          energyAfter: l.energyAfter,
      ...
      for (const ins of generated.insights) {
        await db.insert(brainInsights).values({
          ...
          actionableTip: ins.actionableTip,
        })
      }

      await db.update(studentProfiles).set({
        studentType: generated.studentType,
        focusCliff: generated.focusCliff,
        peakHours: generated.peakHours,
        peakDays: generated.peakDays,
        processingStyle: generated.processingStyle,
